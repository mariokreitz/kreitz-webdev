...
import type { IPublicCompanyRepository } from '@app/database/interfaces/public-company.repository.interface';
...
function buildRepository(records: PublicCompanyRecord[]): IPublicCompanyRepository {
  return {
...
function buildService(records: PublicCompanyRecord[] = [record]): {
  service: PublicCompanyService;
  repository: IPublicCompanyRepository;
...
describe('PublicCompanyService', () => {
  describe('getCompanies', () => {
    it('returns the website companies in the documented public shape with exactly id, name, role, logoUrl, startDate, and endDate', async () => {
      const { service } = buildService([record]);

      const result = await service.getCompanies('website-1');
    ...
    it('queries the repository with exactly the given websiteId, so no code path can return another website data', async () => {
      ...
      await service.getCompanies('website-a');
    ...
    it('never calls the repository with a websiteId other than the one it received', async () => {
      ...
      await service.getCompanies('website-b');

      expect(repository.findManyByWebsiteId).not.toHaveBeenCalledWith('website-a');
    });

    it('returns an empty array when the website has no companies, so the caller can render nothing', async () => {
      const { service } = buildService([]);

      const result = await service.getCompanies('website-c');

      expect(result).toEqual([]);
    });

    it('logs an info event with the websiteId and the returned company count, never the company data itself', async () => {
      const { service, logger } = buildService([record]);

      await service.getCompanies('website-1');

      expect(logger.info).toHaveBeenCalledTimes(1);
      expect(logger.info).toHaveBeenCalledWith({
        event: 'public_company.listed',
        websiteId: 'website-1',
        count: 1,
      });
    });
    ...
    describe('caching', () => {
      it('populates the cache under the website:{id}:companies key on the first read', async () => {
        const { service, cacheService } = buildService([record]);

        await service.getCompanies('website-1');

        expect(cacheService.getOrSet).toHaveBeenCalledWith(
          buildWebsiteCompaniesCacheKey('website-1'),
          60_000,
          expect.any(Function),
        );
      });
      ...
      it('re-fetches from the repository after the cache entry is deleted', async () => {
        ...
        await service.getCompanies('website-1');
        await cacheService.del(buildWebsiteCompaniesCacheKey('website-1'));
        await service.getCompanies('website-1');
      ...
      it('caches each website under its own independent key', async () => {
        ...
        await service.getCompanies('website-1');
        await service.getCompanies('website-2');
