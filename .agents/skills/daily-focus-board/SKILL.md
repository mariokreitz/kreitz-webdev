...
kept current _by conversation_: the human tells you what they finished, you update the board.

This is Ember partnership in daily practice: not a static to-do list, a thing you run _with_
...
sub-note, and an optional tag. If a task is a _count toward a number_ (steps, pages,
...
```html
<script>
  window.__BOARD__ = null; /* SKILL: replace null with the config object above */
</script>
```
...
- `quad` (optional) sets a task's Eisenhower priority: `iu` (important & urgent → _Do first_),
  `ins` (important, not urgent → _Schedule_), `niu` (urgent, not important → _Delegate_), `ninu`
  (neither → _Later_). Renders as a colored accent; the person can change it on the board.
...
- When the user says they finished / started something, either (a) tell them the one-tap move
  ...
  the _task list itself_ changes (add/rename tasks) — status and notes live in the browser
...
- **Beat activation energy: shrink the first step.** When someone's stuck starting a task, don't
  say "just do it." Offer _one tiny concrete first action_ ("open the doc and write the ugliest
...
- **Never shame an incomplete.** No "you didn't finish." Offer **"not today"** carryover freely —
  deciding _not_ to do something is a valid, healthy choice, not a failure. Missing one task
...
- **Protect against overload.** The board nudges gently when it passes ~9 active tasks; back it
  ...
  focus board.
- **Open with a check-in, not a task list.** Invite them to _locate_ how they're arriving
...
- **Prioritize together, gently.** If everything feels equally urgent, walk the Eisenhower
  ...
  point is to make _Schedule_ (important, not urgent) visible, not to cram more in.
...
Frame all of this as _executive-function-friendly design for everyone_ — never diagnose, never
...
- **State lives in the browser (localStorage).** It's per-browser and you (the agent) can't read
  ...
  copied recap to you, you _can_ journal and plan from it. For a fully automatic read/write loop,
