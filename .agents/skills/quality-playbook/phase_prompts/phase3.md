...
**Step 1. Enumerate the authoritative item set.** Mechanical extraction from source — uapi header, spec section, documented constants. Do NOT invent. Example: for VIRTIO*F_RING_RESET-family, grep `include/uapi/linux/virtio_config.h` for `VIRTIO_F*\*` and list the bits the REQ covers.
