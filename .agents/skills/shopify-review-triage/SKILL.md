...
3. **Keyword output is a sort, not a verdict.** Everything produced by the rubric alone is
   labeled _first pass - not human-checked_. Only a person who read the review and checked it
   against their own systems may relabel an item _human-checked_.
...
- record the outcome as _reproduced_, _not reproduced_, or _attempted - notes attached_.

Ask for these outcomes rather than assuming them. Until you have them, every item stays labeled
_first pass - not human-checked_, including in the summary line. An unverified P0 is a candidate,
not an incident.
...
Open the summary line with the counts, e.g. _"Triaged 8 rows supplied: 3 incident risk,
...
- [ ] Every unverified item says _first pass - not human-checked_; nothing claims a human check
      that did not happen.
