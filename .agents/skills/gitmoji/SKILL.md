...
- **scope** _(optional)_ — the section of the codebase affected, in parentheses
...
- Add a body (separated by a blank line) only when the _why_ is not obvious from the subject
...
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Mixed changes (e.g. feature + refactor in one diff) | Pick the emoji for the _dominant_ intent, and suggest splitting into separate commits if the concerns are unrelated                           |
