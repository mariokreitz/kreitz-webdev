...
const recent = await db.orm.posts.select('title', 'authorId', 'createdAt').orderBy({ createdAt: -1 }).take(10).all();
```
...
await db.orm.users.where({ _id: user._id }).update((u) => [u.tags.push('admin'), u.loginCount.inc(1)]);
...
```typescript
await runtime.execute(db.query.from('users').insertOne({ name: 'Alice', email: 'a@e.com', bio: null }));
