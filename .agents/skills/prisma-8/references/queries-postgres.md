...
await db.orm.User.where((u) =>
  and(or(u.kind.eq('admin'), u.email.ilike('%@example.com')), not(u.posts.none((p) => p.title.ilike('%draft%')))),
).all();
```
...
await db.orm.User.include('posts', (posts) => posts.combine({ recent: posts.take(3), total: posts.count() })).all();
...
const summary = await db.orm.User.select('id', 'email', 'kind').create({ id, email, displayName, kind, createdAt });
...
```typescript
const revenue = await db.orm.Sale.where((s) => s.day.gte(start)).aggregate((a) => ({ total: a.sum('amount') }));
