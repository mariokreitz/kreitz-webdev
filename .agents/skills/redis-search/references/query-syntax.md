...
These are the single biggest source of empty-result bugs. TAG values are _not_ tokenized; hyphens, dots, commas, `@`, `:`, and spaces inside a tag must be escaped with a leading backslash, and the whole value lives inside `{}`.
...
Unquoted multi-word values in a `@field:` clause split on whitespace and AND the terms _across_ the index, not scoped to the field. Use `"…"` for an exact phrase or `(…)` for word-AND scoped to the field.
