...
Narrowing the candidate set by NUMERIC, TAG, or GEO criteria rather than text relevance. Filtering happens in the query expression itself (the left side of a query like `(@type:{mountain} @price:[100 500])=>[KNN ...]`) and prunes documents _before_ the more expensive scoring stages.
