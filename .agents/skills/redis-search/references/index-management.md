...
Use index _aliases_ so applications query a stable name while you swap the underlying index on schema changes. `FT.ALTER` can append fields to an existing index but cannot change a field's type, options, or remove it — anything beyond _adding_ a field requires building a new index and swapping the alias.
