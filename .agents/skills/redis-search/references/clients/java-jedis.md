...
This reference covers the `FT.*` (Redis Search) surface of the Jedis client. It shows how Jedis _expresses_ the canonical CLI form — it does not re-explain the query DSL. Read it after a reference that already states _what_ to do.
...
- Field names in the schema are the **hash field names verbatim** — no `$.` path, no `.as("alias")` call. The schema field's name _is_ the alias.
...
For the query DSL itself (delimiters, operators, escaping), read [`../search-syntax-primitives.md`](../search-syntax-primitives.md). This section shows only how Jedis _binds_ a query to `FT.SEARCH`.
...
**Argument order gotcha:** `.apply(expression, alias)` puts the expression _first_. redis-py uses the opposite order via keyword: `apply(discounted="@price * 0.9")` — keyword _is_ the alias. When porting from Python, swap.
