...
> auto-loads). It encodes the _canonical commands_ and the _phase-gating rules_
...
**phase `<N>`** (the first _lit_ phase). **Enforcement is a separate manual
...
protected branches>`), CI _runs_ on PRs but does **not** _block_ merges.
...
starting the next phase.** Never base a phase branch or its PR on a _sibling_
...
> rule** so it actually _blocks_ merges is a platform-UI/admin action
