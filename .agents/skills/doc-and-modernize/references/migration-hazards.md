...
A generic, **stack-agnostic** catalog of migration hazards that are _predictable
per ecosystem_ but are routinely missed in a first-draft modernization plan.
...
> These are _hazard classes_, not a fixed checklist for one stack. The examples
...
3. Fold the **plan action** into the phase's tasks and exit criteria _before_
...
of the classes below. Running this catalog _is_ that critic, institutionalized.
...
**Risk:** A module you _didn't_ list still transitively depends on the removed
...
- _JVM:_ Spring Cloud 2023 removes Netflix Hystrix/Turbine; `monitoring` and
...
- _JS:_ dropping `request` — grep for every `require('request')` / `from
'request'`, not just the one service you remembered.
- _Python:_ removing a C-extension lib — check every `import` site + extras.
...
(not a patch/minor). Major bumps carry _mechanical_ breaking changes independent
of your own logic.
...
- _Spring Boot 2→3:_ `javax.* → jakarta.*` namespace migration **and** JUnit 4→5
  (add `junit-vintage-engine` to keep JUnit 4 tests running, migrate
  `OutputCapture`-style Jupiter-only cases). Tool: **OpenRewrite**.
- _JS:_ CommonJS→ESM, or React class→hooks. Tools: **jscodeshift**, `react-codemod`.
- _Python:_ `2to3`/`pyupgrade`; `async` API renames.
- _.NET:_ `Newtonsoft`→`System.Text.Json`, `HttpClient` patterns.
...
and then fails to _run_ (bytecode/ABI/syntax mismatch), often only in a
...
- _JVM:_ source-built services emit JDK 21 bytecode but every `Dockerfile` still
  `FROM java:8-jre` → containers won't start; dev-compose still _built_ the
  quarantined `auth-service`.
- _Node:_ CI bumped to Node 22 but Docker image still `node:14-alpine`.
- _Python:_ venv on 3.12 but the container base is `python:3.7`.
...
one or more _other_ traffic classes — anonymous flows (signup, password reset,
...
- _BFF/OAuth2:_ SCG BFF + TokenRelay covered login but missed the **anonymous
  registration POST + signup assets**, and `/uaa/users` was `permitAll` instead
  of requiring a narrow `user.write` service scope.
- _Metrics:_ resource-server security must keep `/actuator/prometheus` open or
...
- _MongoDB 3→7:_ direct image bump over an existing anonymous `/data/db` volume
  ...
  rollback.
- _Postgres major:_ needs `pg_upgrade` or dump/restore, not just a tag change.
...
flag this, and which later phase closes it? Is the weakening _minimally_ scoped
...
that _describes_ topology: agent-instructions file, README, architecture doc's
...
- [ ] **H1** Grepped the dead artifact across _all_ manifests; full quarantine set listed?
...
- [ ] **H5** Stateful-store major has a migration path _or_ an explicit destructive-reset decision + rollback?
