...
describes _the exact code in front of you_ rather than whatever `main` happens
...
Local-first is not local-_never_-remote: a web/API lookup is a deliberate
...
- **Commands & Verification Inventory** — a table of the canonical project
  ...
  _enforced_** — i.e. whether any workflow is a **required status check /
...
- **Deployment & Runtime Surface** — a table of every place the language/runtime
  and backing-service versions are pinned _for running_ the system (not just
...
- How this project relates to its broader ecosystem or sibling services, _as
  visible from disk_ (build tags, optional linked repos, separately-deployable
...
from outside the local tree. The reader asked for _this codebase_, documented
faithfully.
...
before touching anything. That "freeze-then-lift" approach is the _lucky_ case,
...
system actually is. On a truly dead app, building a green legacy gate _is itself
a modernization project_ — a circular trap this skill is designed to avoid.
...
reminder that _enforcing_ CI as a required check is a manual human step, not
...
criteria are only worth anything if they are _runnable_, so the plan must be able
...
_how alive is this system, what migration shape fits, and how much regression
safety can we actually achieve?_

##### 2a. Run a time-boxed feasibility spike

Put a **hard time box** on it (e.g. one day). The spike's question is **NOT "can
we make CI green?"** — it is **"can we get this to run even _once_ to capture its
...
it is an _output_ of modernization, not an input.** So every plan must name, up
...
- **Post-testability ("lit") regime:** it builds and runs on a live, supported
  stack. _Now_ real CI, characterization, and e2e are achievable and worth the
...
- **(A) Freeze-then-lift.** Net the old app _as-is_, then upgrade under the net.
  Requires the old app to be **resurrectable at acceptable cost** — i.e. it
  _almost_ runs. This is the traditional "characterization tests before you
...
- **(B) Beachhead-then-expand (walking skeleton).** Don't net the corpse. Drive
  ...
  first phase is _"reach the Testability Milestone with the smallest slice,"_ not
  _"freeze the whole legacy toolchain."_

If a component **can't be built or run at all**, characterization tests on the
_old_ stack are impossible by definition — invert the approach: **stop trying to
net the corpse and build the net on the _target_ stack incrementally**, using the
...
**Anchor safety at the behavioral _seams_, not the unit.** The durable,
...
_those_ as golden-master / approval snapshots — **they survive the rewrite; unit
tests on dead legacy code get deleted the moment you replace the module.** Get an
**oracle**, ranked by what's available:

- **Best — it runs _somewhere_** (prod, a VM, an old container, even read-only),
  **and you're allowed to use it**: record real I/O and replay it against the new
  code. One successful capture beats a permanent green legacy CI. _(Do not assume
...
- **Self-frozen golden master — no external reference exists or is permitted:**
  when nothing trustworthy runs _and_ you may not borrow an external oracle, the
  **modernized component records its _own_ golden master the moment it first runs**
  ...
  it guarantees the _self-consistency of later refactors_, **not** the correctness
...
Just as the Testability Milestone names _when a component can first run and be
tested_, the **CI Milestone** names _when continuous integration is first stood
up_ — and it must be **stated out loud in the roadmap, not buried in a phase
...
**Rule: CI is stood up in the _first lit phase_ — at or immediately after the
component crosses its Testability Milestone — never before.** CI cannot be green
on a component that cannot yet build/run, so wiring a CI _gate_ during the dark
...
2. **Make that workflow an _enforced_ gate** — a **required status check /
   ...
   configures it, CI _runs_ on PRs but does **not** _block_ merges.
...
necessary but **not sufficient**. Modernization plans also fail on _tactical,
ecosystem-predictable_ hazards — an incomplete quarantine set, a forgotten
...
- **Transitive-quarantine completeness (H1).** When removing/cutting over a whole
  ...
  _full_ set of dependents that must move or be quarantined — not the subset you
...
- **Runtime↔deployment lockstep (H3).** If a recommendation bumps the
  language/runtime **major**, every place the runtime is pinned for _running_ the
...
**The first phase establishes maximum _achievable_ safety — not a fixed "green
...
- **Strategy A (Freeze-then-lift), component is resurrectable:** the first phase
  _is_ the classic Safety Net & Baseline — purely additive, no behavior change:
...
- **Name the CI Milestone in the roadmap.** State explicitly which phase stands
  up CI (the first _lit_ phase, at/after the Testability Milestone) so the reader
  never has to guess when CI arrives — and flag that _enforcing_ it (required
...
- **Merge to trunk before the next phase; never stack (H7).** Each phase branch
  ...
  starts**. Never base a phase branch or its PR on a _sibling_ phase branch — that
...
- **Stand up CI at the CI Milestone, and hand off enforcement to a human.**
  ...
  _runs_ but does not _block_ merges — so record "enable branch protection /
...
- **Oracle & seam contracts** — which running instance / recorded I/O / golden
  ...
  diffed against them
- **Testing strategy** — what new test coverage is needed (built on the _target_
...
3. **Feasibility spike result & strategy** — per component: spike findings, the
   ...
   **safety-ladder rung**, and a **residual-risk register** for anything below L4;
   ...
   _enforcing_ it is a manual human step
4. Target architecture with ADRs
5. Per-feature migration analysis (with testability status + safety rung)
6. Phased implementation plan — first phase establishes the highest _achievable_
...
- **Resolve every in-phase decision during planning.** A phase plan must carry
  ...
  _stakeholder/business_ choices (budget, team size, product direction, timeline
...
- **Name the CI Milestone, hand enforcement to a human.** State which phase
  ...
  _enforced_ required check / branch-protection rule is a manual platform step
...
- **Keep it generic.** Express commands and gates in terms of _the project's_
