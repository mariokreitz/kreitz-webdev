...
const tools = mc?.getTools ? await mc.getTools() : await navigator.modelContextTesting?.listTools(); // removed 2026-07; legacy only
```
...
For **declarative** tools also verify the _synthesized_ schema: the form-control →
...
2. Valid example executes: assert the result per `expect` — `expect.result` as a
   ...
   state as a **delta** (capture the relevant state _before_ executing; mere
...
Schema-level verification proves tools _work_, not that an LLM _picks_ them.
...
- **Model Context Tool Inspector** Chrome extension (by Google's François
  Beaufort): natural-language smoke tests of tool _selection_.
