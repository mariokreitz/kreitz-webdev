...
result — never right after _starting_ the action. Agents plan from what is on
...
handle.ready.then((ok) => {
  if (!ok) console.warn('WebMCP tools not active');
});
```
...
export function syncToolsForUser(user: User | null) {
  dispose?.();
  const tools = [...publicTools, ...(user ? memberTools : []), ...(user?.role === 'admin' ? adminTools : [])];
