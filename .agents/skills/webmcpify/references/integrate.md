...
>
  ...
  <input type="email" id="email" name="email" required toolparamdescription="Email address for the reply" />
...
form.addEventListener('submit', (e) => {
  e.preventDefault();
  const result = doSubmit(new FormData(e.target)).then(() => 'Request received. Reply within one business day.');
...
- In `onSubmit`, the WebMCP fields live on the NATIVE event:
  ...
  if (native.agentInvoked)
    native.respondWith?.(
      doSubmit(new FormData(e.currentTarget)).then(() => 'Request received. Reply within one business day.'),
    );
  ```
