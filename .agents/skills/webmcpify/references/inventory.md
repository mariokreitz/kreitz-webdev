...
- `declarative` — any standard `<form>` whose fields map 1:1 to the action's
  inputs: plain HTML, SSG-emitted, server-rendered, _and_ framework-rendered forms
...
nav), enumerate what a user can _do_ per screen, and diff against the manifest.
