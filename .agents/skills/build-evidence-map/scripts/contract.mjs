...
export function inspectMapContract(map) {
  ...
  if (typeof map.updatedAt === 'string' && updatedAt === null) {
    findings.push(finding('$.updatedAt', 'map-date', 'updatedAt must be a real calendar date in YYYY-MM-DD format.'));
  }
  ...
  for (const [index, node] of nodes.entries()) {
    ...
    if (!NODE_TYPES.has(node.type)) {
      findings.push(finding(`${base}.type`, 'node-type', `Node type must be one of: ${[...NODE_TYPES].join(', ')}.`));
    }
  ...
  for (const [index, source] of sources.entries()) {
    ...
    for (const key of ['title', 'publisher', 'date', 'retrievedAt', 'url', 'locator', 'excerpt']) {
      if (!source[key] || typeof source[key] !== 'string') {
        findings.push(finding(`${base}.${key}`, 'source-field', `${key} must be a non-empty string.`));
      }
    }
    ...
    if (typeof source.date === 'string' && sourceDate === null) {
      ...
      findings.push(finding(`${base}.date`, 'future-source-date', 'Source date cannot be later than map.updatedAt.'));
    }
    ...
    if (source.verification != null) {
      ...
      if (!verification || typeof verification !== 'object' || Array.isArray(verification)) {
        findings.push(finding(verificationBase, 'verification-type', 'verification must be an object.'));
      } else {
  ...
  for (const [index, edge] of edges.entries()) {
    ...
    if (uniqueEdges.has(edgeKey)) {
      findings.push(finding(base, 'duplicate-edge', 'Duplicate from/to/relation edges are not allowed.'));
    } else {
      uniqueEdges.add(edgeKey);
    }
    if (!RELATIONS.has(edge.relation)) {
      findings.push(
        finding(`${base}.relation`, 'edge-relation', `Relation must be one of: ${[...RELATIONS].join(', ')}.`),
      );
    }
    if (!edge.note || typeof edge.note !== 'string') {
      findings.push(finding(`${base}.note`, 'edge-note', 'Each reasoning edge needs a plain-language note.'));
    }
  ...
  for (const [index, node] of nodes.entries()) {
    ...
    if (node.type === 'evidence' && !node.sourceId) {
      findings.push(finding(`${base}.sourceId`, 'unsourced-evidence', 'Evidence nodes require sourceId.'));
    }
    if (node.sourceId && !sourceIds.has(node.sourceId)) {
      findings.push(finding(`${base}.sourceId`, 'unknown-source', `Node references unknown source: ${node.sourceId}.`));
    }
    if (node.type === 'evidence' && node.id && !edges.some((edge) => edge?.from === node.id)) {
      findings.push(finding(base, 'unused-evidence', 'Evidence must participate in at least one reasoning edge.'));
    }
  }

  for (const [index, source] of sources.entries()) {
    if (source?.id && !nodes.some((node) => node?.type === 'evidence' && node.sourceId === source.id)) {
      findings.push(
        finding(`$.sources[${index}]`, 'unused-source', 'Every source must be attached to at least one evidence node.'),
      );
    }
  }
  ...
  if (positions.length !== 1) {
    findings.push(finding('$.nodes', 'position-count', 'The map must contain exactly one position node.'));
  ...
  for (const nodeId of cyclicNodes) {
    ...
    findings.push(finding(`$.nodes[${index}]`, 'reasoning-cycle', `Node ${nodeId} participates in a reasoning cycle.`));
  }
