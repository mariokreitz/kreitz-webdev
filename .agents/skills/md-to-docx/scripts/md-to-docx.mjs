        ...
        new TableRow({
          children: row.map(
            (cell) =>
              new TableCell({
                shading:
                  idx % 2 === 1 ? { type: ShadingType.SOLID, color: TABLE_ALT_BG, fill: TABLE_ALT_BG } : undefined,
                children: [
                  new Paragraph({
...
for (const tok of tokens) {
  ...
  if (tok.depth === 1 && mainTitle !== 'Document' && decodeEntities(tok.text || '').includes(mainTitle)) continue;
...
for (const token of tokens) {
  switch (token.type) {
    case 'heading': {
      ...
      if (token.depth === 1 && mainTitle !== 'Document' && decodeEntities(token.text || '').includes(mainTitle)) {
        continue;
      }
    ...
    case 'paragraph': {
      ...
      const imgToken =
        token.tokens && token.tokens.length === 1 && token.tokens[0].type === 'image' ? token.tokens[0] : null;
      if (imgToken) {
