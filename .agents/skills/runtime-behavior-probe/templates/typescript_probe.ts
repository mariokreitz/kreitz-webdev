...
export async function importRepoModule(repoRelativePath: string): Promise<Record<string, unknown>> {
...
function runtimeContext(): Record<string, unknown> {
  const approvedEnvVars = Object.fromEntries(
    APPROVED_ENV_VARS.map((name) => [name, process.env[name] ? 'set' : 'unset']),
  );
...
function startCase(caseId: string, options: { mode?: ProbeMode; note?: string } = {}): void {
  emit('case_start', {
    case_id: caseId,
  ...
  const summarizedCases = Object.fromEntries(
    [...cases.entries()].map(([caseId, items]) => {
      ...
      const resultFlags = Object.fromEntries(
        [...new Set(effectiveItems.map((item) => item.result_flag))].map((flag) => [
          flag,
      ...
      return [
        caseId,
        {
          ...
          observations: effectiveItems.slice(0, 3).map((item) => item.observation_summary),
        },
      ];
    }),
  );
