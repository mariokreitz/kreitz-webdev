...
function sortKeys(value: unknown): unknown {
  ...
  for (const [key, entry] of Object.entries(value).sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0))) {
...
async function processPackage(manifestPath: string): Promise<Result[]> {
  ...
  try {
    ...
    throw new Error(`${manifestPath}: not valid JSON (${error instanceof Error ? error.message : String(error)})`);
  }
  ...
  out.push(await emit(manifestPath, raw, replaceMigrationHash(strippedManifestRaw, strippedOldHash, newHash)));
