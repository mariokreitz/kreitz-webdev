...
function contractNeedsPublicDefaultMigration(raw: string): boolean {
  return raw.includes('"kind": "postgres-unbound-schema"') || raw.includes('"kind":"postgres-unbound-schema"');
}
...
async function packageHasBuildContractSpace(pkgPath: string): Promise<boolean> {
  ...
  try {
    ...
    return typeof scripts['build:contract-space'] === 'string' && scripts['build:contract-space'].length > 0;
  } catch {
    return false;
  }
}
...
async function patchMigrationToHash(migrationTsPath: string, storageHash: string): Promise<boolean> {
