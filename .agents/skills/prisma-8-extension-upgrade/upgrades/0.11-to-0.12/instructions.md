...
function renderExpr(expr: AnyExpression): string {
  switch (expr.kind) {
    ...
    // v8 ignore next 4
    default:
      throw new Error(`Unsupported expression node kind: ${(expr satisfies never as { kind: string }).kind}`);
  }
}
```
...
it('forwards verifyMarker option to createRuntime', async () => {
  ...
  await db.connect(/* … */);
  expect(mocks.createRuntime).toHaveBeenCalledWith(expect.objectContaining({ verifyMarker: false }));
});
...
> = Omit<
  ReturnType<typeof baseDefineContract<MyFamily, MyTargetPack, Types, Models, ExtensionPacks, Capabilities>>,
  'target' | 'targetFamily'
> & {
...
type MyTargetBaseScaffold<
  ...
  Capabilities extends Record<string, Record<string, boolean>> | undefined,
> = Omit<
  ContractInput<MyFamily, MyTargetPack, Record<never, never>, Record<never, never>, ExtensionPacks, Capabilities>,
...
export function defineContract<
  ...
  const ExtensionPacks extends Record<string, ExtensionPackRef<'sql', string>> | undefined = undefined,
...
type MyTargetBaseScaffold<ExtensionPacks extends Record<string, ExtensionPackRef<'sql', string>> | undefined> = Omit<
...
export function defineContract<
  ...
  const Models extends ModelsConstraint = Record<never, never>,
  const ExtensionPacks extends Record<string, ExtensionPackRef<'sql', string>> | undefined = undefined,
>(definition: MyTargetDefinition<Types, Models, ExtensionPacks>): MyTargetResult<Types, Models, ExtensionPacks>;
```
