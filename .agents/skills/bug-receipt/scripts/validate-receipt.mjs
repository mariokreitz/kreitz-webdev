...
export function validateReceipt(receipt) {
  ...
  if (receipt.evidenceSource !== undefined && !evidenceSources.has(receipt.evidenceSource))
    add('evidenceSource', 'Must be executed-now, supplied, or mixed.');
  if (receipt.version === 2 && !evidenceSources.has(receipt.evidenceSource))
    add('evidenceSource', 'Version 2 requires an evidence source.');
  ...
  if (!isObject(receipt.rootCause)) {
    ...
    if (!Array.isArray(receipt.rootCause.evidence)) {
      ...
      receipt.rootCause.evidence.forEach((entry, index) => {
        ...
        if (!nonEmpty(entry.observation))
          add(`rootCause.evidence[${index}].observation`, 'Must be a non-empty string.');
      });
    }
  }
  ...
  if (!Array.isArray(receipt.verification)) {
    ...
    receipt.verification.forEach((entry, index) => {
      ...
      if (!verificationResults.has(entry.result))
        add(`verification[${index}].result`, 'Must be passed, failed, or not-run.');
      if (!nonEmpty(entry.evidence)) add(`verification[${index}].evidence`, 'Must be a non-empty string.');
    });
  }

  if (!Array.isArray(receipt.gaps) || receipt.gaps.some((gap) => !nonEmpty(gap)))
    add('gaps', 'Must be an array of non-empty strings.');

  if (receipt.status === 'verified') {
    if (receipt.baseline?.result === 'not-run') add('baseline.result', 'Verified requires an observed baseline.');
    if (!Array.isArray(receipt.rootCause?.evidence) || receipt.rootCause.evidence.length === 0)
      add('rootCause.evidence', 'Verified requires concrete root-cause evidence.');
    if (!Array.isArray(receipt.changes) || receipt.changes.length === 0)
      add('changes', 'Verified requires at least one changed file or artifact.');
    if (!Array.isArray(receipt.verification) || receipt.verification.length === 0)
      add('verification', 'Verified requires at least one verification check.');
    if (Array.isArray(receipt.verification) && receipt.verification.some((entry) => entry?.result !== 'passed'))
      add('verification', 'Every verification check must pass for verified status.');
    if (Array.isArray(receipt.gaps) && receipt.gaps.length > 0)
      add('gaps', 'Verified status cannot contain proof gaps.');
  }

  if (receipt.status === 'partial' && Array.isArray(receipt.gaps) && receipt.gaps.length === 0)
    add('gaps', 'Partial status must name at least one missing proof layer.');
  if (receipt.status === 'blocked' && Array.isArray(receipt.gaps) && receipt.gaps.length === 0)
    add('gaps', 'Blocked status must name the external blocking condition.');
