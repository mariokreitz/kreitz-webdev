...
function targetUrl(raw, allowQuery) {
  ...
  if (target.search && !allowQuery) {
    throw new CliError(
      '--url has a query string; verify it contains no secret, then pass --allow-query',
...
function validateArtifact(format, contentType, body) {
  ...
  if (contentType !== expected) {
    throw new CliError(
      `expected ${expected} but received ${contentType || 'no content type'}`,
...
export async function run(argv, options = {}) {
  ...
  try {
    ...
    const safe = error instanceof CliError ? error : new CliError(redact(error.message), 'unexpected_error', 5);
