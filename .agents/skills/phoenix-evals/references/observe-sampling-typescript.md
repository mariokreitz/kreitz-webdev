...
const latency = (s: (typeof spans)[number]) => new Date(s.end_time).getTime() - new Date(s.start_time).getTime();
...
const flaggedSpanIds = new Set(annotations.filter((a) => a.result?.label === 'hallucinated').map((a) => a.span_id));
