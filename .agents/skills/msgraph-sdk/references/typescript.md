...
const user: User = await graphClient.api('/me').select('displayName,mail,jobTitle').get();
```
...
const pageIterator = new PageIterator(graphClient, firstPage, (message: Message) => {
...
await graphClient.api(`/me/drive/root:/${fileName}:/content`).putStream(content);
```
...
```typescript
import { Client, RetryHandlerOptions, RetryHandler, MiddlewareFactory } from '@microsoft/microsoft-graph-client';
...
const response: MessageCollectionResponse = await graphClient.api('/me/messages').select('subject,from').get();
