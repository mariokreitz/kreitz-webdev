...
async function ghApi(args) {
  try {
    ...
    if (err?.code === 'ENOENT')
      throw new Error('`gh` CLI not found. Install GitHub CLI and authenticate (gh auth login).');
  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(reviews) && reviews.length) {
        ...
        out.review = rev ? (rev.state || '').toUpperCase() : (reviews[reviews.length - 1].state || '').toUpperCase();
      } else {
...
function buildMarkdown(prs, label) {
  ...
  for (const pr of prs) {
    lines.push(
      `**[${pr.title}](${pr.html_url})** · \`${pr.repo}\` · ${pr.status} · ${pr.review} · CI: ${pr.ci} · ${pr.createdAt}\n\n`,
    );
...
async function renderHtml(md, label = 'PR Dashboard', prs = []) {
  ...
  function statusColor(s) {
    return (
      { DRAFT: '#848d97', MERGED: '#6f42c1', CLOSED: '#da3633', OPEN: '#d29922' }[(s || '').toUpperCase()] || '#848d97'
    );
  }
  function reviewColor(r) {
    return (
      { APPROVED: '#2ea043', CHANGES_REQUESTED: '#da3633', REVIEW_REQUIRED: '#d29922' }[(r || '').toUpperCase()] ||
      '#848d97'
    );
  }
  function ciColor(c) {
  ...
  const rows = prs
    .map((pr) => {
      const previewHtml =
        pr.bodyHtml && String(pr.bodyHtml).trim()
          ? pr.bodyHtml
          : pr.summary
  ...
  try {
    replaced = replaced.replace(/<title>[^<]*<\/title>/, `<title>PR Dashboard — ${escapeHtml(label)}</title>`);
  } catch (e) {}
  try {
    replaced = replaced.replace(/<h1[^>]*>[^<]*<\/h1>/, `<h1>🔀 PR Dashboard — ${escapeHtml(label)}</h1>`);
  } catch (e) {}
