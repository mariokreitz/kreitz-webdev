...
export function CardGrid({ cards }: { cards: { id: number; title: string }[] }) {
  return (
    ...
    >
      {cards.map((card) => (
        <motion.div key={card.id} variants={cardVariants} className="bg-white rounded-xl p-6 shadow-sm border">
          <h3>{card.title}</h3>
        </motion.div>
      ))}
    </motion.div>
...
export function TiltCard({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div ref={ref} style={{ perspective: '1000px' }}>
      <motion.div style={{ rotateX, scale, opacity }} className="bg-white rounded-2xl p-8 shadow-lg">
        {children}
      </motion.div>
    </div>
  );
